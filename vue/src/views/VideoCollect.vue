<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <!-- <el-upload action="http://localhost:9090/videoCollect/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
    ...
    <el-table :data="tableData" :header-cell-class-name="'headerBg'" border stripe
      ...
      <el-table-column label="文章" prop="videoId"></el-table-column>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogFormVisible" title="信息" width="40%">
      <el-form ref="ruleForm" :model="form" :rules="rules" label-width="140px" size="small" style="width: 85%;">
        <el-form-item label="文章" prop="videoId">
  ...
  methods: {
    ...
    save() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          this.request.post("/videoCollect", this.form).then(res => {
            if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/videoCollect/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/videoCollect/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/videoCollect/export")
    },
