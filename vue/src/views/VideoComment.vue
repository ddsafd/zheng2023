<template>
  <div>
    ...
    <el-table :data="tableData" :header-cell-class-name="'headerBg'" border stripe
      ...
      <el-table-column label="视频" prop="videoName"></el-table-column>
...
export default {
  ...
  methods: {
    save2() {
      this.request.post("/videoComment", this.form).then(res => {
        if (res.code === '200') {
    ...
    load() {
      this.request.get("/videoComment/page", {
        params: {
    ...
    del(id) {
      this.request.delete("/videoComment/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/videoComment/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    exp() {
      window.open("http://localhost:9090/videoComment/export")
    },
