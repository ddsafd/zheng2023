<template>
  <div style="padding: 10px">
      ...
      <el-col :span="24">
        <el-card>
          <div class="item1" style="margin: 0 0 0 25px">
            <div>
              <span style="font-size: 23px;font-weight:bold;color: #E6A23C">{{ video.name }}</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              <el-button v-if="state" size="small" type="success" @click="collect">收藏</el-button>
              <el-button v-if="!state" size="small" type="danger" @click="collect">取消收藏</el-button>
            </div>
            <div style="margin-top: 20px">
              ...
              <span style="color: #666666">收藏数</span>&nbsp;&nbsp;:&nbsp;&nbsp;<span
                style="color: #222">{{ video.collectCount }}</span>&nbsp;&nbsp;&nbsp;&nbsp;
            </div>
          </div>
        </el-card>
      </el-col>
    ...
    <!--          <img :src="video.img" :preview-src-list="[video.img]"-->
    ...
    <el-row :gutter="10">
      <el-card style="margin-top: 15px">
        <div class="video-container">
          <video ref="videoPlayer" :src="video.file" controls></video>
        </div>
        ...
        <div style="margin-top: 15px" v-html="video.content"></div>
    ...
    <!--          <video ref="videoPlayer" :src="video.file" controls width="100%" height="560"></video>-->
    <!--        </div>-->
    <!--      </el-card>-->
    <!--    </el-row>-->
...
export default {
  ...
  duration: 500  // 设置为1.5秒
      ...
      let data = {videoId: this.id, userId: this.user.id}
      this.request.post("/videoCollect", data).then(res => {
        if (res.code === '200') {
          this.state = false;
    ...
    load() {
      this.request.get("/video/" + this.id).then(res => {
        this.video = res.data
      })

      this.request.get("/videoCollect/repeat/" + this.id + '/' + this.user.id).then(res => {
        if (res.code === '200') {
          this.state = true;
        } else {
          this.state = false
  ...
  duration: 500  // 设置为1.5秒
    ...
    load2() {
      this.request.get("/videoComment/tree", {
        params: {
          foreignId: this.id,
        }
      }).then(res => {
  ...
  duration: 500  // 设置为1.5秒
      ...
      this.replyComment.videoId = this.id;
      this.request.post("/videoComment", this.replyComment).then(res => {
        if (res.code === '200') {
  ...
  duration: 500  // 设置为1.5秒
      ...
      this.comment.videoId = this.id
      this.request.post("/videoComment", this.comment).then(res => {
        if (res.code === '200') {
